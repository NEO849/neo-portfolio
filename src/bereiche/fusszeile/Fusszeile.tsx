    ...
    <footer className="border-t border-white/[0.06] mt-24">
      <div className="max-w-6xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
          ...
          {/* Navigation */}
          <div>
            <p className="font-mono text-xs text-white/35 mb-3 uppercase tracking-widest">
              Navigation
            </p>
              ...
              {SOZIALE_LINKS.map((link) => (
                <li key={link.label}>
                  <a
                    ...
                    className="flex items-center gap-2 text-sm text-white/55 hover:text-akzent-400 transition-colors duration-200 group"
                  >
                    <span className="w-1 h-1 rounded-full bg-white/20 group-hover:bg-akzent-400 transition-colors" />
                    <span>{link.label}</span>
                    <span className="font-mono text-xs text-white/35">({link.kurz})</span>
                  </a>
                </li>
              ))}
            </ul>
